...
\end{lemma}
\begin{proof}
       \underline{Forward implication:} Suppose $(r)$ is prime and $r \neq 0$. Since prime ideals are proper, $(r) \neq R$. 
       
       Therefore $r$ is not a unit. If $r|ab$, then $ab \in (r)$, so $a \in (r)$ or $b \in (r)$. Thus $r|a$ or $r|b$. So $r $ is prime.

       \underline{Reverse implication:} $\left\{0\right\}\unlhd R$ is a prime ideal since $R$ is an integral domain. Let $r \in R$ be a prime. $(r)\neq R$ since $r \in R^{\times }$. 

       If $ab \in (r)$ then $r|ab$. 
       \begin{align*}
...
\begin{example*}
       ...
       .\] $R$is a subring of a field, so it's an integral domain. Define a function (called the \emph{norm}) \[
...
\begin{definition*}[Principal ideal domain]
       ...
       Example: $\mathbb{Z}$ is a PID by Lemma [todo].
\end{definition*}
\begin{proposition}
...
\end{lemma}
\begin{proof}
       \underline{Forward implication:} $r \notin R^{\times}$ so $(r)\neq r$. Suppose $(r) \subset J \leq R$. Since $R$ is a PID, $J=(a)$ for some $a \in R$. Therefore $r=ab$ for some $b \in R$. Since $r$ is irreducible, either $a \in R^{\times } \implies J=R$ or $b \in R^{\times } \implies J= (r)$. Thus $(r)$ is maximal. 

       \underline{Reverse implication:}
\end{proof}
\end{document}