...
\begin{proof}
      ...
      \begin{equation*}
            \begin{split}
               0 \leq ||tu-v||^2 &=<tu-v,tu-v>\\
               &=t \overline{t} <u,u>-t<u,v>-\overline{t} <v,u>+||v||^2\\
               &=|t|^2 ||u||^2-2 \operatorname{Re}(t<u,v>)+||v||^2.
            \end{split}
      \end{equation*}
      We now choose explicitly 
      \[t=\frac{\overline{<u,v>} }{||u||^2}.\]
      This gives 
      \[0 \leq \frac{|<v,u>|^2}{||u||^4}||u||^2-2 \operatorname{Re}\left(\frac{|<u,v>|^2}{||u||^2}\right)+||v||^2\]
      \[\implies 0 \leq ||v||^2-\frac{|<v,u>|^2}{||u||^2}\]
      \[\implies |<v,u>|^2 \leq ||v||^2 ||u||^2.\]
\end{proof}
\begin{remark}
...
\begin{definition}[Orthogonal/orthonormal family]
      ...
      \begin{itemize}
           \item[(i)] \vocab{orthogonal} if $<e_i,e_j> =0 \ \forall i \neq j$
           \item[(ii)] \vocab{orthonormal} if $<e_i,e_j> =\delta_{ij} \ \forall i,j$
      \end{itemize}
\end{definition}
...
\begin{lemma}
      ...
      \begin{itemize}
           ...
           \[\lambda_j=\frac{<v,e_j> }{||e_j||^2}.\]
      \end{itemize} 
\end{lemma}
\begin{proof}
     From the definitions:
     \begin{itemize}
          \item[(i)]\[\sum_{i=1}^{k}\lambda_i e_i=0 \implies 0=<\sum_{i=1}^{k}\lambda_i e_i,e_j> =\sum_{i=1}^{k}\lambda_i <e_i,e_j>= \lambda_j ||e_j||^2 \implies \lambda_j=0 \forall 1 \leq j \leq k. \]
          \item[(ii)] If $v=\sum_{i=1}^{k}\lambda_i e_i$, then 
          \[<v,e_j> =<\sum_{i=1}^{k}\lambda_i e_i,e_j>=\sum_{i=1}^{k}\lambda_i <e_i,e_j> =\lambda_j ||e_j||^2 .\]
          \[\implies \lambda_j=\frac{<v,e_j> }{||e_j||^2}.\]
     \end{itemize}
...
\begin{lemma}[Parseval's identity]
      ...
      \[<u,v> =\sum_{i=1}^{n}<u,e_i> \overline{<v,e_i> } .\]
      In particular, 
...
\begin{proof}
     ...
     \[u= \sum_{i=1}^{n}<u,e_i>  e_i, \ v= \sum_{i=1}^{n}<v,e_i> e_i.\]
     Therefore 
     \[<u,v> =<\sum_{i=1}^{n}<u,e_i>  e_i,\sum_{i=1}^{n}<v,e_i> e_i>=\sum_{i=1}^{n}<u,e_i> \overline{<v,e_i> }.\]
...
\begin{proof}
      ...
      \begin{equation*}
          \begin{split}
               <e_{k+1},e_j>&=<v_{k+1}-\sum_{i=1}^{k}<v_{k+1},e_i>e_i,e_j>\\
               &=<v_{k+1},e_j> - \sum_{i=1}^{k}<v_{k+1},e_j> \underbrace{<e_i,e_j>}_{=\delta_{ij}}\\
               &=<v_{k+1},e_j>-<v_{k+1},e_j>=0.
          \end{split}
      \end{equation*}
...
\begin{definition}[Orthogonal direct sum]
      ...
      \begin{enumerate}
           ...
           \item $\forall (v_1,v_2) \in V_1 \times V_2: \ <v_1,v_2>=0 $.
      \end{enumerate}
\end{definition}
\begin{remark}
